...
$query = "SELECT tahsin.*, 
                 ...
                 ustad.nama_ustad
          FROM tahsin
          INNER JOIN unit ON tahsin.id_unit = unit.id_unit
          LEFT JOIN surah ON tahsin.id_surah = surah.id_surah
          LEFT JOIN juz ON tahsin.id_juz = juz.id_juz
          LEFT JOIN ustad ON tahsin.id_ustad = ustad.id_ustad";
...
<body class="sb-nav-fixed">
                ...
                <div class="container-fluid px-4">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <a href="exporttahsin.php" class="btn btn-info">Export data</a>
                        </div>
                                ...
                                <table id="datatablesSimple" class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php foreach ($tahsinData as $data): ?>
                                            <tr>
                                                <td><input type="checkbox" name="selected_ids[]" value="<?= htmlspecialchars($data['id_tahsin']); ?>"></td>
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal<?= $data['id_tahsin']; ?>">Edit</button>
                                                    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#DeleteModal<?= $data['id_tahsin']; ?>">Hapus</button>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
                                <button type="submit" class="btn btn-danger" name="delete_tahsin">Delete</button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <?php foreach ($tahsinData as $data): ?>
    <div class="modal fade" id="editModal<?= $data['id_tahsin']; ?>" tabindex="-1" aria-labelledby="editModalLabel<?= $data['id_tahsin']; ?>" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel<?= $data['id_tahsin']; ?>">Edit Data tahsin</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="post" action="tahsin.php">
                    <div class="modal-body">
                        <input type="hidden" name="id" value="<?= htmlspecialchars($data['id_tahsin']); ?>">
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary" name="update_tahsin">Simpan</button>
...
<!------------------------ Hapus banyak ------------------------>
    <!-------------------------------------------------------------->
    <div class="modal fade" id="deleteModal<?= $data['id_tahsin']; ?>" tabindex="-1" aria-labelledby="DeleteModalLabel<?= $data['id_tahsin']; ?>" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="DeleteModalLabel<?= $data['id_tahsin']; ?>">Hapus Data tahsin</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                ...
                <div class="modal-footer">
                    <form method="post" action="tahsin.php">
                        <input type="hidden" name="id" value="<?= htmlspecialchars($data['id_tahsin']); ?>">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-danger" name="delete_tahsin">Hapus</button>
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="post" action="tahsin.php">
                        <input type="hidden" name="id" value="<?= htmlspecialchars($data['id_tahsin']); ?>">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-danger" name="deletetahsin">Hapus</button>
                    </form>
                </div>
            </div>
        </div>
